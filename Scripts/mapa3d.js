...
window.initMapa3D = function() {
    ...
    const planetData = {
        imperium: [
            { color: 0xffdf00, pos: [-8, 0, 5], size: 1.5, speed: 0.005 }, // Terra/Marte
            { color: 0xffdf00, pos: [-25, 5, 20], size: 1.2, speed: 0.003 }, // Cadia (destruído mas presente espiritualmente rs)
            { color: 0xffdf00, pos: [8, -5, 30], size: 1.4, speed: 0.004 }, // Macragge
            { color: 0xffdf00, pos: [-40, -2, 10], size: 1.1, speed: 0.006 }, // Armageddon
            { color: 0xffdf00, pos: [-15, -10, -25], size: 1.3, speed: 0.005 }, // Fenris
            { color: 0xffdf00, pos: [15, 10, 15], size: 1.2, speed: 0.004 } // Baal
        ],
        tau: [
            { color: 0x3be0ff, pos: [28, -2, 18], size: 1.4, speed: 0.006 }, // T'au
            { color: 0x3be0ff, pos: [40, 6, 28], size: 1.1, speed: 0.004 }, // Pech
            { color: 0x3be0ff, pos: [20, -12, 40], size: 1.3, speed: 0.005 }, // Vior'la
            { color: 0x3be0ff, pos: [50, 0, 15], size: 1.2, speed: 0.007 }, // Sa'cea
            { color: 0x8b008b, pos: [45, -15, 25], size: 1.5, speed: 0.002, beingDevoured: true } // Planeta T'au sendo devorado pela Leviathan
        ],
        ork: [
            { color: 0x4cff1a, pos: [-30, -6, -30], size: 1.6, speed: 0.007 }, // Planeta Fábrica
            { color: 0x4cff1a, pos: [-48, 10, -18], size: 1.4, speed: 0.005 }, // Planeta de Escravos
            { color: 0x4cff1a, pos: [-12, 15, -40], size: 1.5, speed: 0.006 } // Planeta Guerra
        ],
        necron: [
            { color: 0x00ff00, pos: [18, 18, -30], size: 1.5, speed: 0.002 }, // Mandragora
            { color: 0x00ff00, pos: [35, -15, -20], size: 1.3, speed: 0.003 } // Solemnace
        ],
        chaos: [
            { color: 0xff1a1a, pos: [110, 30, -110], size: 1.8, speed: 0.004 }, // Sortiarius (Planeta dos Feiticeiros)
    ...
    Object.keys(planetData).forEach(faction => {
        ...
        planetsConfig.forEach(data => {
            ...
            let geo;
            if (data.isCraftworld) {
                geo = new THREE.TorusGeometry(data.size, data.size/3, 16, 50);
            } else if (data.hasSpikes) {
                geo = new THREE.IcosahedronGeometry(data.size, 0); // Fica pontudo!
            } else {
    ...
    const tyranidTargets = [
        new THREE.Vector3(...planetData.tau[4].pos) // Focar toda a frota Leviathan no planeta T'au!
    ];
